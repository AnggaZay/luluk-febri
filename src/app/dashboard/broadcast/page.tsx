            ...
            {linksData.length === 0 ? (
              ...
              linksData.map((guest) => (
                <div key={guest.id} className="p-4 md:p-6 hover:bg-stone-50/50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4 group">
                  ...
                  <button onClick={() => handleSendWA(guest)} className={`px-6 py-3 rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 ${guest.is_sent ? 'bg-stone-100 text-stone-500 hover:bg-stone-200' : 'bg-green-600 text-white hover:bg-green-700 shadow-md shadow-green-900/10'}`}>
                    ...
                    {guest.is_sent ? "Kirim Ulang WA" : "Kirim Via WA"}
                  </button>
                </div>
              ))
            )}
