    ...
    <div className="mobile-journey-container relative h-[100dvh] w-full bg-[#fdfbf7] flex flex-col overflow-hidden">
      ...
      <div className="relative h-[50dvh] w-full pt-10 pb-2 px-6">
        {data.stories.map((_, i) => (
          <div key={`gallery-${i}`} className={`mobile-gallery-${i} absolute inset-0 flex items-center justify-center gap-3 p-6 ${i === 0 ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 10 + i }}>
            {/* Foto Kiri */}
            <div className="relative w-1/2 aspect-square rounded-xl overflow-hidden shadow-2xl border-[6px] border-white bg-stone-200 rotate-[-4deg]">
            ...
            <div className="relative w-1/2 aspect-square rounded-xl overflow-hidden shadow-2xl border-[6px] border-white bg-stone-200 rotate-[3deg] translate-y-6">
      ...
      <div className="relative h-[50dvh] w-full p-6 pb-12">
        {data.stories.map((story, i) => (
          <div key={`story-${i}`} className={`mobile-story-${i} absolute inset-6 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-stone-100 p-6 flex flex-col items-center justify-center text-center ${i === 0 ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 10 + i }}>
